...
const AdminProductList = () => {
    ...
    // URL 쿼리 파라미터에서 현재 페이지 가져오기 (기본값 1)
    const page = Number(searchParams.get("page")) || 1;

    const fetchProducts = async (pageNum: number) => {
        try {
            setLoading(true);
            const data = await getProducts(pageNum, 10); // limit 10 고정
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <Button
                    variant="primary"
                    size="sm"
                    onClick={() => navigate("/admin/products/new")} // 등록 페이지로 이동
                >
                    + 상품 등록
                </Button>
            </div>
                                ...
                                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-400">{product.colors?.[0]?.productCode} 등</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex gap-1 flex-wrap">
                                            ...
                                            {product.colors.length === 0 && <span className="text-gray-400 text-xs">-</span>}
                                        </div>
                            ...
                            총 {product.colors.length}개 컬러
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <div className="flex flex-col gap-1 items-center">
                                            {product.isNew && <span className="text-[10px] bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded font-bold">NEW</span>}
                                            {product.isBest && <span className="text-[10px] bg-red-100 text-red-800 px-2 py-0.5 rounded font-bold">BEST</span>}
                                            {!product.isNew && !product.isBest && <span className="text-gray-400 text-xs">-</span>}
                                        </div>
                                    </td>
